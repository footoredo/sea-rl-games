...
class VectorizedReplayBuffer:
    def __init__(self, obs_shape, action_shape, capacity, device):
        ...
        self.is_dict_obs = type(obs_shape) == dict
        if self.is_dict_obs:
            self.obses = { k: torch.empty((capacity, *v), dtype=torch.float32, device=self.device) for k, v in obs_shape.items() }
            self.next_obses = { k: torch.empty((capacity, *v), dtype=torch.float32, device=self.device) for k, v in obs_shape.items() }
        else:
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        num_observations = action.shape[0]
        ...
        if remaining_capacity < num_observations:
            if self.is_dict_obs:
                for k in self.obses.keys():
                    self.obses[k][0: overflow] = obs[k][-overflow:]
                    self.next_obses[k][0: overflow] = next_obs[k][-overflow:]
            else:
        ...
        if self.is_dict_obs:
            for k in self.obses.keys():
                self.obses[k][self.idx: self.idx + remaining_capacity] = obs[k][:remaining_capacity]
                self.next_obses[k][self.idx: self.idx + remaining_capacity] = next_obs[k][:remaining_capacity]
        else:
    ...
    def sample(self, batch_size):
        ...
        if self.is_dict_obs:
            obses = { k: self.obses[k][idxs] for k in self.obses.keys() }
            next_obses = { k: self.next_obses[k][idxs] for k in self.obses.keys() }
        else:
